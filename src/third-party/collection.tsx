...
import type {NotionContext} from '../context';
...
const CollectionViewBlock: React.FC<{
	readonly block: types.CollectionViewBlock | types.CollectionViewPageBlock;
	readonly className?: string;
}> = ({block, className}) => {
...
const CollectionViewTabs: React.FC<{
	readonly collectionViewId: string;
	readonly viewIds: string[];
	readonly onChangeView: (viewId: string) => unknown;
...
const CollectionViewColumnDesc: React.FC<{
	readonly collectionView: types.CollectionView;
	readonly className?: string;
